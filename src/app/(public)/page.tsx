...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetchCategorias();
    fetchObjetivos();
  }, [fetchProdutos, fetchCategorias, fetchObjetivos]);

  const produtosExibidos = filteredProducts.length > 0 ? filteredProducts : [];

  return (
    <Center paddingBottom="40px" flexDirection="column" gap="40px">
      <Box zIndex={0} mt={-4} width="100%">
        <Banner />
      </Box>
      <Box>
        <Heading color="black">Produtos</Heading>
        <Grid
          templateColumns={{
            base: "1fr",
            sm: "repeat(2, 1fr)",
